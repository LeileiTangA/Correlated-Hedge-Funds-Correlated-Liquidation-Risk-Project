function Distance = FcmGrnn_HedgeFund_Outsample(netattack, OutSample)
% The last column of netattack is the clasifier of event: 1=Alive,2=Liquidate
% netattack sample is for calculating the centriod and Outsample (Fund-of-fund) is for
% calcuating the distance for each fof-fund in the OutSample
P1=netattack;
T1=P1(:,end)';
P1(:,end)=[];
...
for nit=1:LoopNum
    ...
    if nit == LoopNum
        for n=1:size(OutSample,1)
            dist1(n)=norm(OutSample(n,:)-cent1);
            dist2(n)=norm(OutSample(n,:)-cent2);
        end
        Distance(1:2,:) = [dist1; dist2];
    end
